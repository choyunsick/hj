...
==표현식이란??
값으로 평가될수 있는 문, 즉 표현식이 평가가되면 결과로
새로운 값이나 기존값을 참조하여야 하며, 위의 리터럴도 값으로 평가가
되기 때문에 리터럴도 표현식입니다
표현식에는 리터럴, 식별자, 연산자, 함수등 값으로 평가될수있는 모든
것들이 표현식입니다

1 + 2 = 3
1 + 2는 표현식이며 평가가 되서 값 3을 가질 수 있습니다
따라서 1 + 2와 3은 같은것 즉 동치입니다
...
예) var sum = 1 + 1; 이 문에서 토큰은 몇개인가?? 7개

1. 원시형 자료(primitive type)
특징) 특정 값이 메모리에 바로 저장된다
값만 저장되는 것으로 따라서 "변경이 불가능한 값"이다
변수에 할당시 실제값이 저장되고, 복사하게 되면
변수에 담은 원시 값은 실제의 값이 복사되어 전달된다

종류 1 : 문자열(string)
문자열의 표현은 '반드시' 
'', "", ``(작은따옴표, 큰따옴표, 백틱(억음부호))로 감싸서 표현합니다
다른 타입과 달리 문자열을 따옴표등으로 감싸는 이유는 
키워드, 예약어, 식별자등 다른 값들과 구분하기 위해서 사용합니다
다만 억음부호는 ES6이후 사용가능
...
사용방법은 '',"" 대신 ``을 사용하는 방법
즉 이말은 따옴표등으로 표현하기에 어려운 대상을 표현한다는 의미

1. 멀티라인 문자열
일반적인 문자열내에서는 사용할 수 없는 줄바꿈(개행), 
...
console.log("unicode : \u00A9"); //유니코드 문자를 사용할수도있다
...
es에 따르면 숫자타입의 값은 배정밀도 64비트 부동소수점 형식을 따라
모든 수를 실수로만 표현가능합니다
이말은 정수등 다른 수의 개념을 표현하기 위한 데이터타입이 없다
정수, 실수, 2진수, 8진수, 16진수등을 모두 10진수로 해석해버림
...
값은 지금 없거나, 넣을수 없는 상황이라 일부러 비워두는 경우 넣는 값
...
특징) 값의 참조값, 주소값만 메모리에 저장되고 실제값은 다른곳에 있음
즉  변수의 메모리 공간에 원시형자료처럼 값이 들어있는 것이 아니라
참조하고 있는 주소의 값이 들어있음 따라서 변경이 가능한 값
따라서 변수에 할당시 (메모리)주소값만 저장되고, 복사하게 되면
